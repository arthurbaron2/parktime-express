...
app.get("/live-data/all", (request, response) => {
    response.status(200).send(parkData);
});
app.get("/live-data/:parkId", (request, response) => {
    const { parkId } = request.params;
    if (!parkId) {
        response.status(400).send("Park ID is required");
    }
    const data = parkData[parkId];
    if (!data) {
...
const fetchAllParksData = async () => {
    let fetchedData = {};
    await Promise.all(parksToFetch.map(async (parkId) => {
